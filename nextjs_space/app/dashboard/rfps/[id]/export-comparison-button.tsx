/**
 * Client Component: Export Comparison Button (STEP 25)
 * Allows exporting supplier comparison in CSV, Excel, or PDF
 */
...
export default function ExportComparisonButton({ rfpId }: ExportComparisonButtonProps) {
  ...
  const handleExport = async (format: 'csv' | 'excel' | 'pdf') => {
    try {
      ...
      const url = `/api/dashboard/rfps/${rfpId}/comparison/export?format=${format}`;
      ...
      a.download = `comparison-${rfpId}-${Date.now()}.${extension}`;
  ...
  return (
    <div className="relative inline-block">
      <button
        ...
        className="flex items-center gap-2 px-4 py-2.5 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 font-medium transition-all shadow-sm hover:shadow"
      >
        {loading ? (
